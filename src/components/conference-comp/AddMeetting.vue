    ...
    <div class="content">
        ...
        <div>
            <el-form :label-position="labelPosition" label-width="80px" :model="from">
                <el-form-item label="会议室名" class="add">
                    <el-input v-model="from.name"></el-input>
                </el-form-item>
                <el-form-item label="地点" class="add">
...
<script>
    import {createMeettingRoom} from '../../request/api';
    ...
    export default {
        ...
        methods: {
            submitForm(from) {
                createMeettingRoom(from).then(res => {
                        if (res.code === 1) {
                            this.$confirm('会议室保存成功', '提示', {
